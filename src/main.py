...
if not TMDB_API_KEY:
    raise RuntimeError("TMDB_API_KEY missing. Put it in .env as TMDB_API_KEY=xxxx") # Don't crash import-time in production
...
# pickle global vars, path config

# 1. Get the location of 'main.py' (inside the 'src' folder)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# 2. Go up one level to the 'Movie-Recommendation-System' folder
PROJECT_ROOT = os.path.dirname(BASE_DIR)

# 3. Point specifically to your 'models' folder
MODELS_DIR = os.path.join(PROJECT_ROOT, "models")

# 4. Now we link the files inside that specific folder
...
# These stay the same (waiting to be filled)
...
def load_pickles():
    ...
    # Load using joblib for better compatibility
...
#TMDB KEYWORD SEARCH ROUTE(MULTIPLE RESULTS)
@app.get("/tmdb/search")
